...
def generar_arbol_limitado(ruta_directorio, archivo_salida, max_nivel=3):
    def recorrer_directorio(ruta, prefijo="", nivel=1):
        ...
        for i, nombre in enumerate(elementos):
            # Ignorar carpetas ocultas o innecesarias
            if nombre.startswith('.') or nombre in {'__pycache__', 'node_modules','env'}:
                continue
    ...
    print(f"Estructura (hasta {max_nivel} niveles, sin carpetas ocultas) guardada en: {archivo_salida}")


# ======= USO =======
ruta_proyecto = "./"
archivo_salida = "estructura_vapi.txt"
